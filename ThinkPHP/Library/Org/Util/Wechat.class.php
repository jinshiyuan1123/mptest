...
<Nonce><![CDATA[%s]]></Nonce>
    ...
    public function setJsapiTicket($jst)
    {
        $this->jsapi_ticket=$jst;
    }
     ...
     */
    public function getForeverMedia($media_id,$path='',$ext=''){
        #if (!$this->access_token && !$this->checkAuth()) return false;
        ...
        $result = $this->http_post(self::API_URL_PREFIX.self::MEDIA_FOREVER_GET_URL.'access_token='.$this->access_token,$data);


        if ($result)
        {
            if($path&&$ext){
            $filename = md5(rand(100,999).time()).'.'.$ext;
            $save_path = $path.$filename;
            if(file_put_contents($save_path,$result)){
...
class ErrorCode
{
     ...
     * @param $contentStr
     */
    public function makeText($FromUserName,$ToUserName,$contentStr){
