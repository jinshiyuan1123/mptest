...
<script>
    ...
    $(function () {
        ...
        function getClinicFeeList(page, pagesize, search, startTime, endTime) {
            $.post("<?php echo U('/WorkloadStatistics/getClinicFee');?>", {
                p: page,
                pagesize: pagesize,
                search: search,
                startTime: startTime,
...
/*                $(".ClinicFeePage_box").html('');
        ...
        function getDrugPurchase(page, pagesize, search, startTime, endTime) {
            $.post("<?php echo U('/WorkloadStatistics/getDrugPurchase');?>", {
                p: page,
                pagesize: pagesize,
                search: search,
                startTime: startTime,
        ...
        $(document).on('click','.export',function(){
            ...
            $.each($('.tabBtn > li'),function(){
                if($(this).hasClass('on')){
                    var load_status = $(this).attr('load_status');
                    if (load_status == 1) {
                        var url = "<?php echo U('/WorkloadStatistics/ClinicFee_export');?>/p/" + _clinicFee_page + "/pagesize/" + _clinicFee_pagesize;
                    } else if (load_status == 2) {
                        var url = "<?php echo U('/WorkloadStatistics/getCareOrderStatistics');?>/action/export/p/" + _careorderstatistics_page + "/pagesize/" + 10;
                    } else if (load_status == 4) {
                        var url = "<?php echo U('/WorkloadStatistics/DrugPurchase_export');?>/p/" + _drugpurchase_page + "/pagesize/" + _drugpurchase_pagesize;
                    } else if (load_status == 5) {
                        var url = "<?php echo U('/WorkloadStatistics/getCollectionStatistics');?>/action/export/p/" + _collectionstatistics_page + "/pagesize/" + 10;
                    }
        ...
        function getCareOrderStatistics(startTime, endTime, name, page) {
           $.post("<?php echo U('/WorkloadStatistics/getCareOrderStatistics');?>",
        ...
        function getCollectionStatistics(startTime, endTime, name, page) {
            $.post("<?php echo U('/WorkloadStatistics/getCollectionStatistics');?>",
                {'startTime':startTime,'endTime':endTime,'name':name,'p':page,'pagesize':10},
                function (data) {
                    if (data.status == 'success') {
                        if (data.data.count > 0) {
